...
$navLinks = [
  ...
  ['text' => 'Dashboard Superviseur', 'icon' => 'dashboard', 'link' => '/superviseur', 'isTitle' => false],
  ['text' => 'Equipe', 'icon' => 'groups', 'link' => '/superviseur/equipe', 'isTitle' => false],
  ['text' => 'Incidents', 'icon' => 'warning', 'link' => '/superviseur/incidents', 'isTitle' => false],
  ['text' => 'Rapports', 'icon' => 'analytics', 'link' => '/superviseur/rapports', 'isTitle' => false],
];
...
<body class="bg-surface font-body text-on-surface">
  ...
  <aside id="sidebarMobile" class="fixed left-0 top-0 z-50 flex h-screen w-64 -translate-x-full flex-col bg-primary shadow-[0_0_24px_rgba(0,7,25,0.22)] transition-transform duration-300 md:hidden">
    <?= require dirname(__DIR__) . DIRECTORY_SEPARATOR . 'layouts/components/asideSupervisor.php' ?>
  </aside>

  <aside class="fixed left-0 top-0 z-50 hidden h-screen w-64 flex-col bg-primary shadow-[0_0_24px_rgba(0,7,25,0.22)] md:flex">
    <?= require dirname(__DIR__) . DIRECTORY_SEPARATOR . 'layouts/components/asideSupervisor.php' ?>
  </aside>
  ...
  <header class="fixed left-0 right-0 top-0 z-40 flex h-16 items-center justify-between bg-[#fef9f1] px-6 md:left-64 md:px-8">
    <div class="flex flex-1 items-center gap-6">
      ...
      <div>
        <div class="text-[10px] font-bold uppercase tracking-[0.24em] text-on-surface-variant">Supervision terrain</div>
    ...
    <div class="hidden items-center gap-3 rounded-full bg-surface-container-highest px-4 py-2 text-xs font-bold text-primary md:flex">
      ...
      <?= count($supervisedGuichets) ?> voie(s) supervisee(s)
    </div>
  </header>
