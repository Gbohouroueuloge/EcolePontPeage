<div class="relative pl-10 flex flex-col gap-1">
  ...
  <p class="text-sm font-headline font-bold text-primary">
    ...
    <span class="uppercase">[<?= htmlspecialchars($activity->immatriculation) ?>]</span>
  </p>
  <p class="text-xs text-on-surface-variant font-mono">
    <?= date('d/m/Y H:i', strtotime($activity->created_at)) ?> •
    <?= number_format((float)$activity->montant, 0, ',', ' ') ?> FCFA •
    Cabine <?= $activity->guichet_id . ' - ' . htmlspecialchars($activity->emplacement) ?>
  </p>
</div>